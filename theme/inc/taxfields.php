...
function rin_define_tax_metas() {
	...
	return apply_filters( 'rin_define_tax_metas',$rin_taxes );
}


// ADDS THE TAXONOMY TERMS IN THE TAXONOMY PAGE
function rin_taxonomy_fieldadd() {
	...
	if ( is_admin() && isset( $_GET['taxonomy'] )) {
		$rin_taxname = $_GET['taxonomy'];
	}
	

	// WALKS THROUGH ARRAYS
	foreach (rin_define_tax_metas() as $rinv){
		if (isset($rinv['taxname']) && $rinv['taxname'] == $rin_taxname) {
...
// Edit term page
function rin_taxonomy_fieldedit($term) {
...
function rin_save_taxmeta( $term_id ) {
	$pr = rin_define_tax_metas();
	$pname = '';
	$tmeta = '';
	foreach ($pr as $value) {
...
if ( isset( $_GET['taxonomy'] )) {
	add_action( $_GET['taxonomy'] . '_add_form_fields', 'rin_taxonomy_fieldadd', 10, 2 );
	add_action( $_GET['taxonomy'] . '_edit_form_fields', 'rin_taxonomy_fieldedit', 10, 2 );
}
if ( isset( $_POST['taxonomy'] )) {
	add_action( 'edited_' . $_POST['taxonomy'] , 'rin_save_taxmeta', 10, 2 );  
	add_action( 'create_' . $_POST['taxonomy'] , 'rin_save_taxmeta', 10, 2 );
}

include('taxfields/taxfields-cases.php');
include('taxfields/taxfields-voice.php');
include('taxfields/taxfields-advice.php');
?>