...
function voice_define_tax_metas() {
	$voice_taxes = array(
		array(
			'taxname' 			=> 'voice-cat',
			'values' 			=> array(
	...
	return apply_filters( 'voice_define_tax_metas',$voice_taxes );
}


// ADDS THE TAXONOMY TERMS IN THE TAXONOMY PAGE
function voice_taxonomy_fieldadd() {
	...
	if ( is_admin() && isset( $_GET['taxonomy'] )) {
		$voice_taxname = $_GET['taxonomy'];
	}
	

	// WALKS THROUGH ARRAYS
	foreach (voice_define_tax_metas() as $rinv){
		if (isset($rinv['taxname']) && $rinv['taxname'] == $voice_taxname) {
...
// Edit term page
function voice_taxonomy_fieldedit($term) {
...
function voice_save_taxmeta( $term_id ) {
	$pr = voice_define_tax_metas();
	$pname = '';
	$tmeta = '';
	foreach ($pr as $value) {
...
if ( isset( $_GET['taxonomy'] )) {
	add_action( $_GET['taxonomy'] . '_add_form_fields', 'voice_taxonomy_fieldadd', 10, 2 );
	add_action( $_GET['taxonomy'] . '_edit_form_fields', 'voice_taxonomy_fieldedit', 10, 2 );
}
if ( isset( $_POST['taxonomy'] )) {
	add_action( 'edited_' . $_POST['taxonomy'] , 'voice_save_taxmeta', 10, 2 );  
	add_action( 'create_' . $_POST['taxonomy'] , 'voice_save_taxmeta', 10, 2 );
}
?>