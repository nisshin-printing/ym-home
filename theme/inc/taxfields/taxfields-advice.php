...
function advice_define_tax_metas() {
	$advice_taxes = array(
		array(
			'taxname' 			=> 'advice-cat',
			'values' 			=> array(
	...
	return apply_filters( 'advice_define_tax_metas',$advice_taxes );
}


// ADDS THE TAXONOMY TERMS IN THE TAXONOMY PAGE
function advice_taxonomy_fieldadd() {
	...
	if ( is_admin() && isset( $_GET['taxonomy'] )) {
		$advice_taxname = $_GET['taxonomy'];
	}
	

	// WALKS THROUGH ARRAYS
	foreach (advice_define_tax_metas() as $rinv){
		if (isset($rinv['taxname']) && $rinv['taxname'] == $advice_taxname) {
...
// Edit term page
function advice_taxonomy_fieldedit($term) {
...
function advice_save_taxmeta( $term_id ) {
	$pr = advice_define_tax_metas();
	$pname = '';
	$tmeta = '';
	foreach ($pr as $value) {
...
if ( isset( $_GET['taxonomy'] )) {
	add_action( $_GET['taxonomy'] . '_add_form_fields', 'advice_taxonomy_fieldadd', 10, 2 );
	add_action( $_GET['taxonomy'] . '_edit_form_fields', 'advice_taxonomy_fieldedit', 10, 2 );
}
if ( isset( $_POST['taxonomy'] )) {
	add_action( 'edited_' . $_POST['taxonomy'] , 'advice_save_taxmeta', 10, 2 );  
	add_action( 'create_' . $_POST['taxonomy'] , 'advice_save_taxmeta', 10, 2 );
}
?>