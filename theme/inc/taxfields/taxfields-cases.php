...
function cases_define_tax_metas() {
	$cases_taxes = array(
		array(
			'taxname' 			=> 'cases-cat',
			'values' 			=> array(
	...
	return apply_filters( 'cases_define_tax_metas',$cases_taxes );
}


// ADDS THE TAXONOMY TERMS IN THE TAXONOMY PAGE
function cases_taxonomy_fieldadd() {
	...
	if ( is_admin() && isset( $_GET['taxonomy'] )) {
		$cases_taxname = $_GET['taxonomy'];
	}
	

	// WALKS THROUGH ARRAYS
	foreach (cases_define_tax_metas() as $rinv){
		if (isset($rinv['taxname']) && $rinv['taxname'] == $cases_taxname) {
...
// Edit term page
function cases_taxonomy_fieldedit($term) {
...
function cases_save_taxmeta( $term_id ) {
	$pr = cases_define_tax_metas();
	$pname = '';
	$tmeta = '';
	foreach ($pr as $value) {
...
if ( isset( $_GET['taxonomy'] )) {
	add_action( $_GET['taxonomy'] . '_add_form_fields', 'cases_taxonomy_fieldadd', 10, 2 );
	add_action( $_GET['taxonomy'] . '_edit_form_fields', 'cases_taxonomy_fieldedit', 10, 2 );
}
if ( isset( $_POST['taxonomy'] )) {
	add_action( 'edited_' . $_POST['taxonomy'] , 'cases_save_taxmeta', 10, 2 );  
	add_action( 'create_' . $_POST['taxonomy'] , 'cases_save_taxmeta', 10, 2 );
}
?>